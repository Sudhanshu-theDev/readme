...
if(isset($_GET['order_id'])){
  ...
  $get_order = mysqli_query($conn, "SELECT * FROM `confirm_order` WHERE id = '$order_id'") or die('query failed');
  if (mysqli_num_rows($get_order) > 0){
    $fetch_order = mysqli_fetch_assoc($get_order);
    
  }
  $get_order = mysqli_query($conn, "SELECT * FROM `orders` WHERE order_id = '$order_id'") or die('query failed');
  if (mysqli_num_rows($get_order) > 0){
  ...
  <body>
    <div class="invoice">
        ...
        <tr>
          ...
          <div class="buyer-details">
              <p class="buyer-name">By, Read Me</p>
...
">
        <table style="width: 100%;">
          ...
          $select_book = mysqli_query($conn, "SELECT * FROM `orders`WHERE order_id = '$order_id'") or die('query failed');
          $s=1;
